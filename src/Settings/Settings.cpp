...
const char DEFAULT_WALLET_FILE_NAME[] = "cryptobidwallet.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("cryptobidwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("cryptobidwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("cryptobidwallet.desktop");
  ...
  res = autorunSettings.contains("cryptobidWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("cryptobidWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.cryptobid.cryptobid scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=cryptobid Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("cryptobidWallet", appPath);
    } else {
      autorunSettings.remove("cryptobidWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\cryptobid", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:cryptobid");
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("cryptobidnwallet.cfg"));
